...
for(i in sp_list){
  ...
  if(nrow(filter(sdm_input, presence == 1)) > 59){
    ...
    # max_ind_pres = dismo::maxent(sdm_input[,c("elev.mean", "bio.mean.bio4","bio.mean.bio5","bio.mean.bio6","bio.mean.bio13","bio.mean.bio14", "ndvi.mean")], ll)
    ...
    sdm_output = cbind(sdm_input, max_pred_pres) 

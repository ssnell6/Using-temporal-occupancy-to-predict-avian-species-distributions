  ...
  if(length(unique(sdm_input$stateroute)) > 40 & length(unique(sdm_input$presence)) >1){
    if(nrow(filter(sdm_input, presence == 1)) > 59){
      ...
      pred_2016 <- left_join(sdm_output, bbs_new_sub[c("stateroute", "presence")], by = "stateroute") %>%
        mutate(predicted_max_pres = ifelse(max_pred_points > threshmax_pres, 1, 0)) %>%
        dplyr::select(aou, stateroute, occ, presence.x, latitude, longitude, predicted_max_pres)
  ...
  if(length(unique(sdm_input$stateroute)) > 40 & length(unique(sdm_input$presence)) >1){
    if(nrow(filter(sdm_input, presence == 1)) > 49){
      ...
      for(j in 1:10){
        ...
        ll_spat <- SpatialPoints(lltrain, proj4string=CRS("+init=epsg:4326 +proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0"))
        ll_spat_train <- spTransform(ll_spat, CRS("+proj=laea +lat_0=45.235 +lon_0=-106.675 +x_0=0 +y_0=0 +ellps=WGS84 +units=km +no_defs"))
         lltest <- data.frame(lon = testData$longitude, lat = testData$latitude)
         ll_spat_test <- SpatialPoints(lltest, proj4string=CRS("+init=epsg:4326 +proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs +towgs84=0,0,0"))
         ll_spat_laea_test <- spTransform(ll_spat_test, CRS("+proj=laea +lat_0=45.235 +lon_0=-106.675 +x_0=0 +y_0=0 +ellps=WGS84 +units=km +no_defs"))
         
        max_ind_pres = dismo::maxent(all_env_raster, ll_spat_train)
        all_env_points <- raster::extract(all_env_raster, ll_spat_laea_test)
        ...
        rmse = rbind(rmse, c(i, rmse_me_pres))
